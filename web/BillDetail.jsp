...
<html lang="en">
  ...
  ================================================== -->
    ...
    <%
        ...
        Vector<Detail> vector1 = (Vector<Detail>) request.getAttribute("detail");
        ...
        for (Bill el : vector) {
            ...
            OrderDate = el.getDate();
            OrderStatus = el.getStatus();
        }
    %>
                ...
                <div class="col-lg-9 title text-center">
                    ...
                    <form action="BillServlet">
                        ...
                        <p>OrderStatus: 
                            ...
                            <select name="status">
                                <% while (rs.next()) { %>
                                <option onchange="this.form.submit()" name = "status" value="<%=rs.getInt(1)%>"><%=status[rs.getInt(1) -1]%></option>
                                <%}%>
                            </select>
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <%for (Detail d : vector1) {%>
                            <tr>
                                <td><%=d.getProductID()%></td>
                                <td><%=d.getProductname()%></td>
                                <td><%=d.getQuantity()%></td>
                                <td><%=d.getListPrice()%></td>
                                <td><%=d.getQuantity() * d.getListPrice()%></td>
                            </tr>
                            <%
                                    Total += d.getQuantity() * d.getListPrice();
                                }%>
                        </tbody>
                    </table>
                    <div style="color: orange; font-size: 20px">Grand Total = $<%=Total%></div>
                </div>
            </div>
        </div>
    </section>
